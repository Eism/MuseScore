...
void RenameChordFBox::undo(EditData*)
{
    ...
    if (!m_onlyRemove) {
        for (auto it = existingDiagramsFromBox.begin(); it != existingDiagramsFromBox.end(); ++it) {
            ...
            if (fd->harmony()->harmonyName() != harmonyName) {
                continue;
            }
...
void RenameChordFBox::redo(EditData*)
{
    ...
    for (Harmony* harmony: findAllHarmonies(m_fretBox->score())) {
        ...
        if (harmony->harmonyName() == m_harmony->harmonyName() && harmony->tick() < currentHarmonyTick) {
            m_onlyRemove = true;
        }

        if (harmony->harmonyName() == m_harmonyOldName) {
            nextMatchHarmonyToReplace = harmony;
            break;
        }

        harmonyBeforeNextMatch = harmony->harmonyName();
    }
    ...
    for (auto it = existingDiagramsFromBox.begin(); it != existingDiagramsFromBox.end(); ++it) {
        FretDiagram* fd = toFretDiagram(*it);
        if (fd->harmony()->harmonyName() != m_harmonyOldName) {
            continue;
        }
        ...
        if (m_onlyRemove) {
            m_diagramsForRestore.push_back(std::make_pair<int, FretDiagram*>(std::distance(existingDiagramsFromBox.begin(), it),
                                                                             toFretDiagram(*it)));

            m_fretBox->remove(fd);
        } else {
            fd->setHarmony(harmonyName);
            fd->updateDiagram(harmonyName);
        }

        if (nextMatchHarmonyToReplace) {
            //! Insert new fret diagram after the harmony preceding the next match
            FretDiagram* fretDiagram = m_fretBox->makeFretDiagram(nextMatchHarmonyToReplace);
            if (fretDiagram) {
                for (size_t i = 0; i < existingDiagramsFromBox.size(); ++i) {
                    FretDiagram* currentFretDiagram = toFretDiagram(existingDiagramsFromBox[i]);
                    if (currentFretDiagram->harmony()->harmonyName() == harmonyBeforeNextMatch) {
                        addFretDiagramToFretBox(m_fretBox, fretDiagram, i + 1);
                        m_diagramForRemove = fretDiagram;
                        break;
                    }
                }
            }
        ...
        for (auto removeIt = existingDiagramsFromBox.end(); removeIt >= it + 1; --removeIt) {
            if (*removeIt && toFretDiagram(*removeIt)->harmony()->harmonyName() == harmonyName) {
                m_diagramsForRestore.push_back(std::make_pair<int, FretDiagram*>(std::distance(existingDiagramsFromBox.begin(), removeIt),
                                                                                 toFretDiagram(*removeIt)));

                existingDiagramsFromBox.remove(*removeIt);
            }
        }
...
void AddChordFBox::undo(EditData*)
{
    ...
    for (auto it = existingDiagramsFromBox.begin(); it != existingDiagramsFromBox.end(); ++it) {
        FretDiagram* fd = toFretDiagram(*it);
        if (!fd || fd->harmony()->harmonyName() != m_chordNewName) {
            continue;
        }
    ...
    for (auto& diagramInfo : m_diagrmsForRestore) {
        int diagramIndex = diagramInfo.first;
        FretDiagram* diagram = diagramInfo.second;
...
void AddChordFBox::redo(EditData*)
{
    ...
    for (Harmony* harmony: findAllHarmonies(m_fretBox->score())) {
        if (harmony->harmonyName() == m_chordNewName) {
        ...
        harmonyBeforeCurrentHarmony = harmony->harmonyName();
    }
    ...
    ElementList& existingDiagramsFromBox = m_fretBox->el();
    for (auto it = existingDiagramsFromBox.begin(); it != existingDiagramsFromBox.end(); ++it) {
        if (!(*it) || !(*it)->isFretDiagram()) {
            continue;
        }

        FretDiagram* fd = toFretDiagram(*it);
        if (!fd || fd->harmony()->harmonyName() != harmonyBeforeCurrentHarmony) {
            continue;
        }
        ...
        addFretDiagramToFretBox(m_fretBox, fretDiagram,
                                std::distance(existingDiagramsFromBox.begin(), it + 1));
        m_added = true;

        //! Remove the following diagram with the new harmony name
        //! +2 because we added one more diagram before
        String harmonyName = currentHarmonyFromScore->harmonyName();
        for (auto removeIt = existingDiagramsFromBox.end(); removeIt >= it + 2; --removeIt) {
            if (*removeIt && toFretDiagram(*removeIt)->harmony()->harmonyName() == harmonyName) {
                m_diagrmsForRestore.push_back(std::make_pair<int, FretDiagram*>(std::distance(existingDiagramsFromBox.begin(), removeIt),
                                                                                toFretDiagram(*removeIt)));
...
void RemoveChordFBox::redo(EditData*)
{
    ...
    for (Harmony* harmony: findAllHarmonies(m_fretBox->score())) {
        if (harmony->harmonyName() == m_chordName) {
            if (harmony->tick() < m_tick) {
    ...
    for (auto it = existingDiagramsFromBox.begin(); it != existingDiagramsFromBox.end(); ++it) {
        FretDiagram* fd = toFretDiagram(*it);
        if (!fd || fd->harmony()->harmonyName() != m_chordName) {
            continue;
        }
        ...
        if (found) {
            ...
            if (fretDiagram) {
                for (size_t i = 0; i < existingDiagramsFromBox.size(); ++i) {
                    FretDiagram* currentFretDiagram = toFretDiagram(existingDiagramsFromBox[i]);
                    if (currentFretDiagram->harmony()->harmonyName() == harmonyBeforeCurrentHarmony) {
                        addFretDiagramToFretBox(m_fretBox, fretDiagram, i + 1);
                        m_addedFretDiagram = fretDiagram;
                        break;
                    }
                }
            }
