...
void NotationToolBarModel::load()
{
    ...
    std::vector<std::string> noteInputActions = std::dynamic_pointer_cast<ToolbarData>(abstractData)->actions;
    ...
    for (const std::string& actionName: noteInputActions) {
        ...
        m_items << makeActionItem(areg->action(actionName), QString::number(section));
    }
...
Action NotationToolBarModel::currentNoteInputModeAction() const
{
    ...
    return aregister()->action(noteInputActionNames[noteInput->state().method]);
}
