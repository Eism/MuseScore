...
static bool workspaceContainsAction(const std::vector<std::string>& workspaceActionNames, const mu::actions::ActionName& actionName)
{
    return std::find(workspaceActionNames.begin(), workspaceActionNames.end(), actionName) != workspaceActionNames.end();
}
...
void NoteInputBarCustomiseModel::load()
{
    ...
    for (const std::string& actionName: actions) {
        ...
        bool checked = workspaceContainsAction(currentWorkspaceActions(), actionName);
...
std::vector<std::string> NoteInputBarCustomiseModel::defaultActions() const
{
    ...
    for (const Action& action: allNoteInputActions) {
        if (workspaceContainsAction(currentWorkspaceNoteInputActions, action.name)) {
            continue;
        }
