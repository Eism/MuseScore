...
static const QString SEPARATOR_LINE_TITLE("-------  Separator line  -------");
...
void NoteInputBarCustomiseModel::load()
{
    ...
    std::vector<std::string> currentWorkspaceActions = this->currentWorkspaceActions();
    auto actionVisibleInCurrentWorkspace = [&currentWorkspaceActions](const actions::ActionName& actionName) -> bool {
                                               return std::find(
                                                   currentWorkspaceActions.begin(), currentWorkspaceActions.end(),
                                                   actionName) != currentWorkspaceActions.end();
                                           };

    for (const std::string& actionName: actions) {
        Action action = actionsRegister()->action(actionName);
        bool checked = actionVisibleInCurrentWorkspace(actionName);
...
{
    bool removingAvailable = m_selectionModel->hasSelection();
...
{
    bool addingAvailable = m_selectionModel->hasSelection() && m_selectionModel->selectedIndexes().count() == 1;
    if (!addingAvailable) {
...
AbstractNoteInputBarItem* NoteInputBarCustomiseModel::makeItem(const mu::actions::Action& action, bool checked)
{
    ...
    item->setTitle(QString::fromStdString(action.title));
    item->setIcon(static_cast<int>(action.iconCode));
...
AbstractNoteInputBarItem* NoteInputBarCustomiseModel::makeSeparatorItem() const
{
    ...
    item->setTitle(SEPARATOR_LINE_TITLE);
    return item;
}

std::vector<std::string> NoteInputBarCustomiseModel::customizedActions() const
{
    std::vector<std::string> result = configurator()->toolbarActions(NOTE_INPUT_TOOLBAR_NAME);

    if (result.empty()) {
        return {};
    }

    auto newAction = [&result](const actions::ActionName& actionName) -> bool {
                         return std::find(
                             result.begin(), result.end(),
                             actionName) == result.end();
                     };

    std::vector<Action> allNoteInputActions = NotationActions::defaultNoteInputActions();
    for (const Action& action: allNoteInputActions) {
        if (actionFromNoteInputModes(action.name)) {
            continue;
        }

        if (newAction(action.name)) {
...
std::vector<std::string> NoteInputBarCustomiseModel::defaultActions() const
{
    ...
    auto actionVisibleInCurrentWorkspace = [&currentWorkspaceNoteInputActions](const actions::ActionName& actionName) -> bool {
                                               return std::find(
                                                   currentWorkspaceNoteInputActions.begin(), currentWorkspaceNoteInputActions.end(),
                                                   actionName) != currentWorkspaceNoteInputActions.end();
                                           };
    ...
    for (const Action& action: allNoteInputActions) {
        if (actionVisibleInCurrentWorkspace(action.name)) {
            continue;
        }
...
std::vector<std::string> NoteInputBarCustomiseModel::currentWorkspaceActions() const
{
    ...
    return std::dynamic_pointer_cast<ToolbarData>(toolbarData)->actions;
}
...
void NoteInputBarCustomiseModel::saveActions()
{
    ...
    configurator()->setToolbarActions(NOTE_INPUT_TOOLBAR_NAME, items);
}
